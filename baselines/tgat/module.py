...
class MergeLayer(torch.nn.Module):
    ...
    def forward(self, x1, x2):
        ...
        #h = self.fc1(x)   # TODO: just linear layer
        return self.fc2(h)
...
MIXTURE_SIZE = 1
edge_raw_embed, node_raw_embed = None, None
class TGAN(torch.nn.Module):
    def __init__(self, ngh_finder, n_feat, e_feat,
                 ...
                 num_layers=3, n_head=4, null_idx=0, num_heads=1, drop_out=0.1,
                 seq_len=None, device=None):
        global edge_raw_embed, node_raw_embed
        ...
        node_raw_embed = torch.nn.Embedding.from_pretrained(n_feat_th, padding_idx=0, freeze=True)
        
        self.feat_dim = n_feat_th.shape[1]
        
        self.n_feat_dim = self.feat_dim
        self.e_feat_dim = self.feat_dim
        self.model_dim = self.feat_dim
        print(self.feat_dim)
        self.rnn = nn.LSTM(1, 64, 2, dropout=0)
        self.post_rnn = nn.Linear(self.feat_dim + 64, self.feat_dim)
        ...
        if use_time == 'time':
            ...
            self.time_encoder = TimeEncode(expand_dim=n_feat_th.shape[1])
        elif use_time == 'pos':
            assert(seq_len is not None)
            self.logger.info('Using positional encoding')
            self.time_encoder = PosEncode(expand_dim=n_feat_th.shape[1], seq_len=seq_len)
        elif use_time == 'empty':
            self.logger.info('Using empty encoding')
            self.time_encoder = EmptyEncode(expand_dim=n_feat_th.shape[1])
        else:
            raise ValueError('invalid time option!')
        
        self.affinity_score = MergeLayer(self.feat_dim, self.feat_dim, self.feat_dim, 1)
        self.embeds_to_log_norm_mix = MergeLayer(self.feat_dim, self.feat_dim,
            self.feat_dim, MIXTURE_SIZE*3)
        self.target_predictor = MergeLayer(self.feat_dim, self.feat_dim,
            self.feat_dim, 16)
        self.clf = nn.Linear(self.feat_dim, 2)

    def binary_predict(self, src_idx_l, cut_time_l, curr_layers,
        num_neighbors=20):
        return self.clf(self.tem_conv(src_idx_l, cut_time_l, curr_layers,
            num_neighbors))
    ...
    def inter_event_pdf(self, src_idx_l, target_idx_l, cut_time_l,
        delta_time_l, first_t_l, num_neighbors=20):
        device = self.device

        src_embed = self.tem_conv(src_idx_l, cut_time_l, self.num_layers, num_neighbors)
        target_embed = torch.zeros_like(src_embed).to(device)
        #target_embed = self.tem_conv(target_idx_l, cut_time_l, self.num_layers, num_neighbors)
        #target_embed = (target_embed.T / torch.norm(target_embed)).T
        #target_embed[:,0] = torch.tensor(cut_time_l).float().to(device)
        #target_embed[:,1] = torch.tensor(delta_time_l).float().to(device)
        #target_embed[:,2] = torch.tensor(first_t_l).float().to(device)
        #target_embed *= 0
        #target_embed[:,0] = torch.tensor(first_t_l).float().to(device)
        params = self.embeds_to_log_norm_mix(src_embed, target_embed)
        params = params.view(-1, 3, MIXTURE_SIZE)
        weights, means, stds = params[:,0,:], params[:,1,:], params[:,2,:]

        #target_pred = self.target_predictor(src_embed, 0*src_embed)
        #target_pred = (target_pred.T / torch.norm(target_pred)).T

        stds = torch.exp(stds)

        #print(delta_time_l.shape)
        delta_time_l = torch.tensor(delta_time_l).float().to(device)
        USE_IFL_TPP = False
        if USE_IFL_TPP:
            delta_time_l = delta_time_l.unsqueeze(1).expand(delta_time_l.shape[0],
                MIXTURE_SIZE)

            gauss = torch.exp(-(torch.log(delta_time_l) - means)**2 / (2*stds**2))
            #print(stds.shape, delta_time_l.shape)

            ts = delta_time_l * stds * np.sqrt(2*np.pi)
            #print(ts.shape)
            p = F.softmax(weights, dim=-1) / ts * gauss
            p = torch.log(torch.sum(p, dim=-1))

        else:
            #scores = torch.sum((src_embed - target_embed)**2, dim=-1)
            means = means[:,0]
            ...
            ##print(means)
            #cut_time_l = torch.tensor(cut_time_l).float().to(device)
            #first_t_l = torch.tensor(first_t_l).float().to(device)

            t = cut_time_l - first_t_l + 1
            t = torch.tensor(t).float().to(device)
            ##means = means*0 + (delta_time_l + 1) / t  # TODO: test
            ##print(t, delta_time_l)
            ##print(n_np)
            ##print(means, t, delta_time_l)
            poi = torch.distributions.poisson.Poisson(means * t)
            #print(delta_time_l, means)
            p = means
            #p = poi.log_prob(delta_time_l + 1)
            #p = torch.max(p, -50.0*torch.ones_like(p)) #+ torch.sum((
                #target_embed - target_pred)**2)
            #p = delta_time_l  # TODO: test
            #print(p)
            #print(p)
            #print(poi)
            #fac = np.math.factorial(n_np)
            #p = torch.pow(means*t, delta_time_l) / fac * torch.exp(-means * t)

        return p

    def inter_event_cdf(self, src_idx_l, target_idx_l, cut_time_l, delta_time_l,
        num_neighbors=20):
        device = self.device

        src_embed = self.tem_conv(src_idx_l, cut_time_l, self.num_layers, num_neighbors)
        target_embed = src_embed# self.tem_conv(target_idx_l, cut_time_l, self.num_layers, num_neighbors)
        params = self.embeds_to_log_norm_mix(src_embed, target_embed)
        params = params.view(-1, 3, MIXTURE_SIZE)
        weights, means, stds = params[:,0,:], params[:,1,:], params[:,2,:]
        ...
        lognorm_cdf = 0.5 + 0.5*torch.erf((torch.log(delta_time_l) - means) /
            stds * np.sqrt(2))
        ps = F.softmax(weights, dim=-1) * lognorm_cdf
    ...
    def tem_conv(self, src_idx_l, cut_time_l, curr_layers, num_neighbors=20):
        ...
        src_node_feat = node_raw_embed(src_node_batch_th).to(device)
        
        USE_RNN = False
        if curr_layers == 0:
            y = src_node_feat
            if USE_RNN:
                x = self.time_series[src_idx_l].to(device)
                n_batches = x.shape[0]
                x = x.unsqueeze(1).permute(2, 0, 1)
                _, (out, _) = self.rnn(x)#, (h0, c0)))
                out = out.view(2, n_batches, 64)
                out = out[-1]
            ...
            src_ngn_edge_feat = edge_raw_embed(src_ngh_eidx_batch).to(device)
